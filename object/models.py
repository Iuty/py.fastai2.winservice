...
class AlexNetS(Model):
    def __init__(self):
        super(AlexNetS, self).__init__()
        self.c1 = Conv2D(filters=32, kernel_size=(3, 3), padding='same',input_shape=[32,96,96,6])#
        
        self.a1 = Activation('relu')
        self.p1 = MaxPool2D(pool_size=(3, 3), strides=2,padding='same')

        self.c2 = Conv2D(filters=256, kernel_size=(3, 3), padding='same')
    ...
    def call(self, x):
        ...
        y = self.f3(x)
        print(r"load model complete shape {}".format(y.shape))
        return y
