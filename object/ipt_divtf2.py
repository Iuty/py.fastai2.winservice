...
#function upon tf.function down
...
def getBatch2(config,image_data,label_data,batch=None):
    imgw = config.Width()
    imgh = config.Height()
    imgd = config.Depth()
    imgf = config.Formatter()
    ...
    temp = np.array([image_data, label_data])
    temp = temp.transpose()
    np.random.shuffle(temp)
    image_list = list(temp[:batch_size, 0])
    label_list = list(temp[:batch_size, 1])
    return image_list,label_list
